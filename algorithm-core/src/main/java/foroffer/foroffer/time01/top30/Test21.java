package foroffer.foroffer.time01.top30;
 ...
 *
 *
 * 对 <T extends Comparable<? super T>> 类型参数的理解
 * 1. T 必须实现 Comparable 接口，(并且这个接口类型是 T 或者任一父类类型)
 * 2. 需要给接口的抽象方法 compareTo 的参数类型为 T 或者其父类类型，也即是 compareTo(T t) 或者 compareTo(? super T)
 * 比如 Dog implements Comparable<Dog>
 *     或者
